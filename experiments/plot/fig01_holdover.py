"""Plot 01: Holdover TINT accumulation -- clean, publication quality."""
import os, sys
...
def main():
    ...
    ax.text(11.8, thr + 14, "0x4 threshold (210 ns)", color=ps.C_REAL,
            fontsize=7, ha="right", va="bottom")

    tc = float(np.interp(thr, full, saat))
    ax.plot([tc], [thr], marker="o", ms=4, color=ps.C_REAL, zorder=5)
    ax.annotate(f"{tc:.1f} h", xy=(tc, thr), xytext=(tc + 0.5, thr - 55),
                fontsize=7, color=ps.C_REAL,
                arrowprops=dict(arrowstyle="-", color=ps.C_REAL, lw=0.6))
