"""
Cizim 01: Holdover birikimi. data/holdover.csv okur; simulasyon calistirmaz.
"""
import os, sys
...
def main():
    ...
    full = np.array([r["tint_ns"] for r in d])
    lin = np.array([r["dogrusal_ns"] for r in d])
    ...
    t_cross = next(r["saat"] for r in d if r["tint_ns"] >= HEALTH_PHASE_THRESHOLD_NS)
    ...
    ax.set_xlim(0, 24); ax.set_ylim(0, full.max() * 1.05)
