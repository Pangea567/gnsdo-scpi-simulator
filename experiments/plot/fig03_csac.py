"""Plot 03: CSAC-PCB fidelity check -- clean."""
import os, sys
...
def main():
    ...
    ax.plot(x, y, color=ps.C_MODEL, lw=1.4,
            label=r"model: CSAC $=$ PCB $+\,1.32\,^\circ$C")
    ax.scatter(pr, cr, color=ps.C_REAL, s=34, zorder=5, edgecolor="white",
               linewidth=0.6, label="recorded device (4 samples)")
    ax.text(0.05, 0.90, f"max residual {resid:.2f}$\\,^\\circ$C",
            transform=ax.transAxes, fontsize=7, color=ps.C_OLD)
    ax.set_xlabel(r"PCB temperature ($^\circ$C)")
    ax.set_ylabel(r"CSAC temperature ($^\circ$C)")
