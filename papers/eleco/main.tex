...
% NOTE: Önceki taslakta üçüncü yazar "Batuhan Canlı" iken e-posta
% "ahmet.canli@tubitak.gov.tr" olarak verilmiştir. Gönderim öncesi doğrulayın.
...
\begin{center}
  ...
  ofkaragoz05@gmail.com, omer.yildiz@tubitak.gov.tr, ahmet.canli@tubitak.gov.tr\par}
  \vspace{18pt}
\end{center}
]
...
\begin{itemize}
    \item Gerçek seri aygıtla aynı erişim biçimini sağlayan sanal seri port, SCPI ayrıştırıcı, tekil cihaz-durumu modeli ve zamana bağlı davranış katmanlarından oluşan modüler bir mimari;
...
Referans cihaz Jackson Labs Technologies Low Noise Rubidium GNSDO'dur \cite{jlt2015}. Üretici, ürün adında GNSDO kısaltmasını kullanmaktadır; bu nedenle makalede ürün adı için GNSDO, genel teknoloji sınıfı için ise GNSS ile disipline edilen osilatör ifadesi kullanılmaktadır. Üretici kılavuzuna göre cihazın RS-232 seri arayüzünün fabrika ayarı 115200 baud hız, 8 veri biti, parite yok, 1 stop biti ve akış kontrolü yok biçimindedir; bu yapı kısaca 8N1 olarak gösterilmektedir. Komut arayüzü SCPI sözdizimini kullanır; SCPI standardı programlanabilir ölçüm cihazları için donanım arayüzünden bağımsız bir komut dili tanımlar \cite{scpi1999}.
...
\subsection{Holdover Sırasında Zaman Aralığı Hatası}
...
\begin{equation}
    ...
    \label{eq:holdover}
\end{equation}
burada $x(t)$ zaman aralığı hatası, $x_0$ holdover başlangıcındaki faz farkı ve $y_h$ etkin artık fraksiyonel frekans hatasıdır.

Üretici kılavuzu \texttt{SYNC:FEEstimate?} değerini 1000~s aralığında Allan varyansına benzer bir büyüklük olarak tanımladığından, bu değer doğrudan işaretli fiziksel frekans sapması olarak yorumlanmamıştır. Simülatörde $|y_h|$ için kaydedilmiş FEE büyüklükleri başlangıç referansı olarak kullanılmakta; işaret ve ölçekleme açık bir model parametresi olarak tutulmaktadır. Bu yaklaşım, kaynak tarafından desteklenmeyen bir fiziksel yorumu modele gömmek yerine varsayımı görünür kılar.
...
\begin{tabular}{p{0.25\columnwidth}p{0.28\columnwidth}p{0.38\columnwidth}}
\toprule
\textbf{Parametre / ilişki} & \textbf{Değer} & \textbf{Kaynak ve durum} \\
\midrule
Seri arayüz & 115200, 8N1 & Kılavuz, doğrudan \\
Sağlık faz koşulu & $|TINT|>210$ ns & Kılavuz, doğrudan \\
...
\midrule
\texttt{*IDN?} & Jackson Labs, LN Rb GNSDO (PRE), Rev 1.17 & Tam \\
...
\texttt{DIAG?} & $EFC_{abs}=200\,EFC_{rel}$ & 4/4 \\
...
Örnek bir $|y_h|=1.8\times10^{-11}$ değeri için Denklem~\ref{eq:holdover}, bir saat sonunda yaklaşık 64.8~ns ve bir gün sonunda yaklaşık \SI{1.56}{\micro\second} zaman aralığı hatası üretmektedir. 210~ns sağlık koşulu yaklaşık 3.24~saatte aşılmaktadır. Şekil~\ref{fig:holdover} bu davranışı göstermektedir.
...
\node[font=\scriptsize,anchor=south west] at (axis cs:3.35,210) {3.24 saat};
\node[font=\scriptsize,anchor=south east] at (axis cs:11.8,210) {210 ns sağlık koşulu};
\end{axis}
\end{tikzpicture}
\caption{$|y_h|=1.8\times10^{-11}$ için doğrusal holdover zaman aralığı hatası}
\label{fig:holdover}
\end{figure}
...
\node[font=\scriptsize,anchor=south east,rotate=90] at (axis cs:19.7,26) {yaklaşık GNSS kilidi};
\end{axis}
\end{tikzpicture}
...
Modelin tekrarlanan çalıştırmalarında aynı başlangıç koşulları için üretilen serilerin maksimum farkı sıfırdır. Bu özellik, zamana bağlı testlerin regresyon ortamında deterministik olarak tekrar edilmesini sağlar.
...
Simülatör, ölçüm/çizim ardışık düzeni ve doğrulamada kullanılan kaydedilmiş cihaz çıktıları \url{https://github.com/Pangea67/gnsdo-scpi-simulator} adresinde sunulmaktadır.
