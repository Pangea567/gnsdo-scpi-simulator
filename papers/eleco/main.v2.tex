...
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{array} % REV: çizelge sütunlarını sola hizalamak için
\usepackage{siunitx}
\usepackage{graphicx}
\usepackage{pgfplots}
\usepackage{tikz}
...
% REV (aaa.v2): aaa.tex üzerinde yalnızca gerekli düzeltmeler yapılmıştır;
% değişen yerler "% REV" ile işaretlidir. Özet:
%  - Kılavuzdaki CSAC = rubidyum MAC ve GPSDO/GNSDO eş anlamlılığı eklendi.
%  - "zaman aralığı farkı" -> "zaman aralığı farkı" (kılavuz ve cihaz
%    çıktısı: "time interval difference").
%  - Zorlanmış holdover kapsamı, |y_h| için süre aralığı, *IDN? metni ve
%    veri kümesi tanımı eklendi / düzeltildi.
%  - Kılavuz-kayıt bulguları ve doğruluk boyutları için iki çizelge eklendi.
%  - Determinizm ve sınırlamaların kapsamı netleştirildi; şekil etiketleri
%    ve Çizelge 1 sütun genişlikleri düzeltildi.
...
% REV: kılavuz §1.1'deki eş anlamlılık notu; CSAC etiketli komutların
% bu cihazda neyi ifade ettiğini açıklar.
...
\label{sec:goals} % REV: Çizelge tab:dims için
...
\subsection{Holdover Sırasında Zaman Aralığı Farkı} % REV: kılavuz terimi
...
% REV: modellenen gözlenebilir büyüklüğün kapsamı (kılavuz §3.6.2).
...
% REV: dar ilk sütundaki kötü hecelemeyi önlemek için genişlikler ve hizalama
...
% REV: önceki taslaktaki kılavuz-cihaz uyumsuzlukları kataloğu kısa biçimde geri eklendi.
...
% REV: veri kümesinin kapsamı açıkça belirtildi.
Karşılaştırmada kullanılan kayıtlar, cihazın büyük bölümünde GNSS'e kilitli olduğu ve alıcının konum ölçümünün (\emph{survey}) sürdüğü yaklaşık 40 dakikalık tek bir oturumda alınmıştır; kayıtların bir kısmı zaman damgası içermemektedir.
...
\midrule
\texttt{*IDN?} & Jackson Labs, LN Rb GPSDO (PRE), Firmware Rev 1.17 & Tam \\ % REV: kayıttaki metinle birebir
...
% REV: yukarıda tek değer yerine FEE aralığına karşılık gelen süre aralığı verildi.
...
\node[font=\scriptsize,anchor=north west] at (axis cs:3.4,195) {3.24 saat}; % REV: etiket çizginin üstünden alındı
...
\node[font=\scriptsize,anchor=south,rotate=90] at (axis cs:19.4,38) {yaklaşık GNSS kilidi}; % REV: etiket görünür konuma taşındı
\end{axis}
\end{tikzpicture}
...
% REV: Bölüm 3'teki doğruluk boyutları sonuçlarla kapatıldı; determinizm
% iddiasının kapsamı (model düzeyi / seri arayüz) netleştirildi.
